 ...
 **/
orm.connect({host:'50.87.248.91', database: 'ogradyjo_ciphercom', user: 'ogradyjo_user', password: 'zt19Zotia01??', protocol:'mysql', port: '3306'}, function(err, db) {
    if (err) { throw err; }
    ormdb = db;
});
/**
    ...
    entry.find({ip: getIP(request)}, 5, ["id", "Z"], function(error, results) {
        ...
        if (!error) {
            ...
            if (!results.length) {
                 ...
                 **/
                /**
                 * @todo send back json data of sql result set
                 */
